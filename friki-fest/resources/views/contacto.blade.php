...
@section('contenido')
    ...
    <div class="card">
        <form method="POST" action="{{ route('contacto.enviar') }}">
            ...
            <input type="text" name="nombre" class="input" value="{{ old('nombre') }}" placeholder="Ej: Luke Skywalker">
            ...
            <input type="email" name="email" class="input" value="{{ old('email') }}" placeholder="friki@galaxia.com">
            @error('email')
                <div class="alerta-error">! {{ $message }}</div>
            @enderror

            <label>Tu mensaje:</label><br>
            <textarea name="mensaje" class="input" rows="4" placeholder="¿Qué opinas del final de Evangelion?">{{ old('mensaje') }}</textarea>
            @error('mensaje')
                <div class="alerta-error">! {{ $message }}</div>
            @enderror
